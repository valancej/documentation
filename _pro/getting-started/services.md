...
Your services files - `codeship-services.yml` - is the [Docker Compose](https://docs.docker.com/compose/) based file your CI/CD process with Codeship relies on to build and orchestrate your environment. At the start of every new build, we create a new build instance, with Docker installed, to run your build on. From there, what containers are built and when is defined by your [codeship-steps.yml]({% link _pro/getting-started/steps.md %}) file running commands on the individual containers defined in your `codeship-services.yml` file.
