...
You can of course always set it directly through the setup commands by running the following command. If the version isn't already pre-installed on the build VMs `nvm` will download the version from the official repositories and make it available to your build.
...
We install the latest versions for **0.8.x**, **0.9.x**, **0.10.x**, **0.11.x**, **0.12.x**, **4.x** and **5.x**.
...
*Note:* Do not run `npm test` to execute grunt tests. When a grunt test fails, it will return a non-zero exit code to `npm`. `npm` will ignore this exit code and return with an exit code of zero. As we compute the status of your tests by taking a look at the exit code of the commands you run, this will cause the command to succeed even though there were failing tests.
