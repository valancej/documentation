...
Your services files - `codeship-services.yml` - is the [Docker Compose](https://docs.docker.com/compose/) based file your CI/CD process with Codeship relies on to build and orchestrate your environment. At the start of every new build, we create a new build instance, with Docker installed, to run your build on. From there, what containers are built and when is defined by your [codeship-steps.yml]({{ site.baseurl }}/docker/steps/) file running commands on the individual containers defined in your `codeship-services.yml` file.
...
The way we encrypt our environment variables is by creating a file in our root directory - in this case, a file named `env` and then [downloading our project AES key.]({{ site.baseurl }}/docker/encryption/) to root directory (and adding it to our `.gitignore` file.)

Once the AES key is in our directory, we can run the `jet encrypt` command with an *input* and an *output* filename: `jet encrypt env env.encrypted` ([Learn more about using Jet]({{ site.baseurl }}/docker/installation/))
...
There are several specific requirements and considerations when using caching, so it is recommended that you [read our caching documentation.]({{ site.baseurl }}/docker/caching//) before enabling caching on your builds.
...
* [Encrypting environment variables.]({{ site.baseurl }}/docker/encryption/)
* [Steps File]({{ site.baseurl }}/docker/steps/)
* [Volumes]({{ site.baseurl }}/docker/docker-volumes/)
...
* [Caching]({{ site.baseurl }}/docker/caching//)
