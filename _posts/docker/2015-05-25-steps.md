...
* `exclude` is the inverse of the `tag` directive. It indicates the branches to exclude in this step.
* `service` or `services` which specify a service or list of services for your steps to run on.
* `steps` is a list of sub-steps.
* `tag` indicates what branches the step will be run on (as a regex).
